...
namespace Graphics
{
    class GPU
    {
        ...
        static public uint GenerateBuffer(Vertex[] data)
        {
            uint BufferID = 0;
            uint[] vbo = { 0 };
            Gl.glGenBuffers(1, vbo);
            ...
            Gl.glBufferData(Gl.GL_ARRAY_BUFFER, (IntPtr)(sizeof(float)*data.Length*Vertex.attributesCount), data, Gl.GL_STATIC_DRAW);
        ...
        static public uint GenerateElementBuffer(Vertex[] data)
        {
            uint BufferID = 0;
            uint[] vbo = { 0 };
            Gl.glGenBuffers(1, vbo);
            ...
            GCHandle Handle = GCHandle.Alloc(data, GCHandleType.Pinned);
            IntPtr Ptr = Handle.AddrOfPinnedObject();
            var size = Marshal.SizeOf(typeof(Vertex)) * data.Length;
            Gl.glBufferData(Gl.GL_ELEMENT_ARRAY_BUFFER, (IntPtr)size, Ptr, Gl.GL_STATIC_DRAW);
            Handle.Free();
            return BufferID;
        }
